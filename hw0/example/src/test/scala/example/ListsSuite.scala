 ...
 */
 class ListsSuite {
   ...
   * when writing tests.
   */
  @Test def `details why one plus one is not two (0pts)`: Unit = {
  ...
  @Test def `sum of a few negative (10pts)`: Unit = {
    assert(sum(List(1,2,-3)) == 0)
  }

  @Test def `sum of a few large (10pts)`: Unit = {
    assert(sum(List(1,500000,0,-1)) == 500000)
  }
  ...
  @Test def `max of a few neg numbers (10pts)`: Unit = {
    assert(max(List(3, 7, 2,-1)) == 7)
    assert(max(List(-3, -7, -2,-1)) == -1)
  }

  @Test def `max of a few big numbers (10pts)`: Unit = {
    assert(max(List(300, 7, 2)) == 300)
  }
